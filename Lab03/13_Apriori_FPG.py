...
def main(filename, min_sup=25, max_sup=60, gaps=5):
    ...
    plt.title('Execution Time vs Minimum Support (pumsb)') ###
    plt.grid(True, linestyle='--', alpha=0.7)
    plt.legend()
    plt.tight_layout()
    plt.savefig('pumsb_time.png') ###
    plt.show()
    ...
    plt.title('Memory Usage vs Minimum Support (pumsb)') ###
    plt.grid(True, linestyle='--', alpha=0.7)
    plt.legend()
    plt.tight_layout()
    plt.savefig('pumsb_mem.png') ###
    plt.show()

if __name__ == "__main__":
    st_time = time.time()
    main('pumsb.dat', 82, 96, 2)
    ed_time = time.time()
